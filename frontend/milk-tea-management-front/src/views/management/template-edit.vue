<template>
  <div class="template-edit-container">
    <el-page-header @back="handleBack" title="返回">
      <template #content>
        <span class="page-title">编辑商品模板</span>
      </template>
    </el-page-header>
...
import { getProductTemplateDetail, updateProductTemplate } from '@/api/product'
...
const loadTemplateDetail = async () => {
  ...
  try {
    ...
    // 处理SKU数据，确保specUnit是枚举值
    const skus = (data.skus || []).map((sku) => ({
      ...sku,
      specUnit: mapSpecUnitToEnum(sku.specUnit),
    }))
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        const res = await updateProductTemplate(submitData)
        if (res.code === 200) {

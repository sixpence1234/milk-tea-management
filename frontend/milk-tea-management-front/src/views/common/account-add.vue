<template>
  <div class="account-add-container">
    <el-page-header @back="handleBack" title="返回">
      <template #content>
        <span class="page-title">创建账号</span>
      </template>
    </el-page-header>
...
const isStore = computed(() => userStore.terminalType === 'STORE')
const isWarehouse = computed(() => userStore.terminalType === 'WAREHOUSE')
...
onMounted(() => {
  // 根据当前用户类型设置默认终端类型和角色
  if (isStore.value) {
    form.terminalType = 'STORE'
    form.userRole = 'EMPLOYEE'
  } else if (isWarehouse.value) {
    form.terminalType = 'WAREHOUSE'
    form.userRole = 'EMPLOYEE'
  }
})
</script>
